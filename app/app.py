...
# Load config
CONFIG_PATH = (
    Path(__file__).resolve().parent.parent / "config" / "visualization_config.yaml"
)
...
def section(title, content, description_md) -> html.Div:
    ...
    return html.Div(
        ...
        children=[
            ...
            dcc.Markdown(
                description_md, style={"color": "#dddddd", "marginBottom": "1rem"}
            ),
            dcc.Graph(figure=content, style={"height": "600px"})
            if is_figure
            else html.Pre(
                content,
                style={
...
# App layout
app.layout = html.Div(
    style={"backgroundColor": "#1e1e1e", "padding": "2rem"},
    children=[
        html.H1(
            "HMM Stock Regime Dashboard",
            style={"textAlign": "center", "color": "#ffffff", "marginBottom": "2rem"},
        ),
        html.Div(
            [
                html.Label(
...
def update_plots(selected_model, selected_ticker, selected_layer):
    ...
    if selected_model in ["LayeredHMMModel", "HierarchicalHMMModel"]:
        return [
            section(
                "Normalized Returns per Regime",
                vis.plot_normalized_return_means(layer=selected_layer),
                descriptions["means_returns"],
            ),
            section(
                "ANOVA Test Results",
                vis.report_anova_test_on_returns(layer=selected_layer),
                descriptions["anova_test"],
            ),
            section(
                "Moving Averages per Regime",
                vis.plot_vol_trend_bar_by_state(layer=selected_layer),
                descriptions["3ma"],
            ),
            section(
                "Feature Means per Regime",
                vis.plot_feature_means(layer=selected_layer),
                descriptions["means"],
            ),
            section(
                "HMM Transition Matrix",
                vis.plot_transition_matrix(layer=selected_layer),
                descriptions["transition"],
            ),
            section(
                "Normalized Returns by Regime",
                vis.plot_time_series_by_regime(layer=selected_layer),
                descriptions["returns"],
            ),
            section(
                "HMM States vs. Volatility",
                vis.plot_states_vs_volatility(layer=selected_layer),
                descriptions["volatility"],
            ),
            section(
                "HMM States vs. Volatility (VIX)",
                vis.plot_regime_capture_of_vol_quantiles(layer=selected_layer),
                descriptions["quantiles"],
            ),
            section(
                "HMM States Correlations",
                vis.plot_state_volatility_correlations(layer=selected_layer),
        ...
        return [
            section(
                "Normalized Returns per Regime",
                vis.plot_normalized_return_means(),
                descriptions["means_returns"],
            ),
            section(
                "ANOVA Test Results",
                vis.report_anova_test_on_returns(),
                descriptions["anova_test"],
            ),
            section(
                "Moving Averages per Regime",
                vis.plot_vol_trend_bar_by_state(),
                descriptions["3ma"],
            ),
            section(
                "Feature Means per Regime",
                vis.plot_feature_means(),
                descriptions["means"],
            ),
            section(
                "HMM Transition Matrix",
                vis.plot_transition_matrix(),
                descriptions["transition"],
            ),
            section(
                "Normalized Returns by Regime",
                vis.plot_time_series_by_regime(),
                descriptions["returns"],
            ),
            section(
                "HMM States vs. Volatility",
                vis.plot_states_vs_volatility(),
                descriptions["volatility"],
            ),
            section(
                "HMM States vs. Volatility (VIX)",
                vis.plot_regime_capture_of_vol_quantiles(),
                descriptions["quantiles"],
            ),
            section(
                "HMM States Correlations",
                vis.plot_state_volatility_correlations(),
                descriptions["correlations"],
            ),
        ]

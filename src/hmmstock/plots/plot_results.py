...
class HMMResultVisualization:
    ...
    def _load_regime_states(self) -> pd.DataFrame:
        csv_file = self.path_manager.get_ticket_csv_file(
            self.model_name, self.ticker, "regime_states.csv"
        )
    ...
    def _load_transition_matrices(self) -> dict:
        ...
        while True:
            csv_file = self.path_manager.get_transition_matrix(
                self.model_name,
                self.ticker,
                f"{self.ticker}_transition_matrix_layer{i}.csv",
            )
            if not csv_file.exists():
                ...
                if i == 0:
                    fallback = self.path_manager.get_ticket_csv_file(
                        self.model_name,
                        self.ticker,
                        f"{self.ticker}_transition_matrix.csv",
                    )
                    if fallback.exists():
    ...
    def get_state_column(self, layer=0) -> str:
        if layer not in self.available_layers:
            raise ValueError(
                f"Layer {layer} not found. Available layers: {self.available_layers}"
            )
    ...
    def plot_normalized_return_means(self, layer=0) -> go.Figure:
        ...
        grouped_means = (
            self.df.groupby(state_col)["normalized_returns"].mean().reset_index()
        )
        ...
        fig = px.bar(
            ...
            labels={
                "normalized_returns": "Mean Normalized Return",
                state_col: "Regime",
            },
        )
    ...
    def plot_feature_means(self, layer=0) -> go.Figure:
        ...
        feature_cols = [
            col for col in numeric_cols if col not in {state_col, "normalized_returns"}
        ]
    ...
    def plot_time_series_by_regime(self, layer=0) -> go.Figure:
        state_col = self.get_state_column(layer)
        color_map = {
            state: px.colors.qualitative.Plotly[
                state % len(px.colors.qualitative.Plotly)
            ]
    ...
    def plot_observation_distributions_by_state(self, layer=0) -> go.Figure:
        ...
        feature_cols = [
            col
            for col in self.df.columns
            if col not in ["Date"] and not col.startswith("regime_state")
        ]
    ...
    def plot_states_vs_volatility(self, layer=0) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis2=dict(
                title="Volatility", overlaying="y", side="right", showgrid=False
            ),
            showlegend=True,
        )
        return fig
    ...
    def plot_vol_trend_bar_by_state(self, layer=0) -> go.Figure:
        ...
        fig.update_layout(
            xaxis_title="Volatility Measure", yaxis_title="Avg 3-Day Trend"
        )
        return fig
    ...
    def report_anova_test_on_returns(self, layer=0) -> str:
        ...
        if p_value < 0.01:
            interpretation = (
                "Highly significant differences between regime return means (p < 0.01)."
            )
        elif p_value < 0.05:
            interpretation = "Statistically significant differences between regime return means (p < 0.05)."
        elif p_value < 0.1:
            interpretation = (
                "Weak evidence of differences in regime return means (p < 0.1)."
            )
        else:
            interpretation = (
                "No significant differences found between regime return means."
            )
